...
def visNormal(myArr,deleteOutlier=True,lineColor="k",histColor="k",nameOfData="original data",partNum=5):
    ...
    ndCoor=np.array([[(ndMin+i*ndPartScale+ndMin+(i+1)*ndPartScale)/2,normArr[np.logical_and.reduce([normArr>=ndMin+i*ndPartScale,normArr<ndMin+(i+1)*ndPartScale])].size] for i in range(partNum)])#get the coordinate of ND 获取随机样本坐标
